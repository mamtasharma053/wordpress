...
if(!function_exists('cpotheme_page_title')){
	function cpotheme_page_title(){
		...
		if(function_exists('is_woocommerce') && is_woocommerce()){
			...
			_e('Archive', 'transcend');
		}elseif(is_404()){
			echo __('Page Not Found', 'transcend');
		}elseif(is_search()){
			echo __('Search Results for', 'transcend').' "'.get_search_query().'"';
		}else{
...
if(!function_exists('cpotheme_edit')){
	function cpotheme_edit(){
		if(cpotheme_get_option('general_editlinks'))
			edit_post_link(__('Edit', 'transcend'));
	}
}
...
if(!function_exists('cpotheme_404')){
	function cpotheme_404(){
		echo apply_filters('cpotheme_404', __('The requested page could not be found. It could have been deleted or changed location. Try searching for it using the search function.', 'transcend'));
	}
}
...
if(!function_exists('cpotheme_footer')){
	function cpotheme_footer(){
		...
		echo '&copy; '.get_bloginfo('name').' '.date("Y").'. '.sprintf(__('<a href="%s">%s</a> theme by CPOThemes.', 'transcend'), esc_url(CPOTHEME_PREMIUM_URL), esc_attr(CPOTHEME_NAME));
		echo '</div>';
	}
}
...
if(!function_exists('cpotheme_sitemap')){
	function cpotheme_sitemap(){
		...
		echo '<h3>'.__('Post Categories', 'transcend').'</h3>';
		echo '<ul>'.wp_list_categories('title_li=&show_count=1&echo=0').'</ul>';
		echo '<h3>'.__('Post Tags', 'transcend').'</h3>';
		...
		if(!is_home() && !is_front_page()){
			...
			if($result == ''){
				...
				if($pid != ''){
					...
					elseif(is_search()):
						$result .= __('Search Results', 'transcend');
					else:
						if(isset($post->ID)) $current_id = $post->ID; else $current_id = false;
						if($current_id){
					...
					$result = "<span class='breadcrumb-separator'></span>";
					$result .= __('Page Not Found', 'transcend');
				}
				$result = '<a class="breadcrumb-link" href="'.home_url().'">'.__('Home', 'transcend').'</a>'.$result;
			}
...
if(!function_exists('cpotheme_search_form')){
	function cpotheme_search_form(){
		if(is_search()){
			...
			echo '<input type="submit" id="search-submit" value="'.__('Search', 'transcend').'" />';
			echo '</form>';
			echo '</div>';

			if(!have_posts()){
				echo '<p class="search-submit">'.__('No results were found. Please try searching with different terms.', 'transcend').'</p>';
			}
		}
	}
}
...
if(!function_exists('cpotheme_postpage_image')){
	function cpotheme_postpage_image(){
		if(has_post_thumbnail()){
			if(!is_singular('post')){
				echo '<a href="'.get_permalink(get_the_ID()).'" title="'.sprintf(esc_attr__('Go to %s', 'transcend'), the_title_attribute('echo=0')).'" rel="bookmark">';
...
if(!function_exists('cpotheme_postpage_author')){
	function cpotheme_postpage_author($display = false, $format_text =''){
		$author_alt = sprintf(esc_attr__('View all posts by %s', 'transcend'), get_the_author());
...
if(!function_exists('cpotheme_postpage_comments')){
	function cpotheme_postpage_comments($display = false, $format_text = ''){
		...
		if($format_text != ''){
			...
			if($comments_num == 0)
				$text = __('No Comments', 'transcend');
			elseif($comments_num == 1)
				$text = __('One Comment', 'transcend');
			else
				$text = __('%1$s Comments', 'transcend');
		}
...
if(!function_exists('cpotheme_postpage_readmore')){
	function cpotheme_postpage_readmore($classes = ''){
		if(!is_singular('post')){
			...
			echo apply_filters('cpotheme_readmore', __('Read More', 'transcend'));
			echo '</a>';
		}
	}
}
	...
	function cpotheme_author(){
		if(get_the_author_meta('description')){
			if(function_exists('ts_fab_add_author_box')){
				...
				if($user_meta != '')
					echo '<a target="_blank" rel="nofollow" class="author-web" href="'.esc_attr($user_meta).'">'.__('Website', 'transcend').'</a>';
				$user_meta = get_the_author_meta('facebook');
				if($user_meta != '')
					echo '<a target="_blank" rel="nofollow" class="author-facebook" href="'.esc_attr($user_meta).'">'.__('Facebook', 'transcend').'</a>';
				$user_meta = get_the_author_meta('twitter');
				if($user_meta != '')
					echo '<a target="_blank" rel="nofollow" class="author-twitter" href="//twitter.com/'.esc_attr($user_meta).'">'.__('Twitter', 'transcend').'</a>';
				$user_meta = get_the_author_meta('googleplus');
				if($user_meta != '')
					echo '<a target="_blank" rel="nofollow" class="author-googleplus" href="'.esc_attr($user_meta).'">'.__('Google+', 'transcend').'</a>';
...
if(!function_exists('cpotheme_pagination')){
	function cpotheme_pagination(){
		...
		//First Page Link
		if(1 == $current_page)
			$out .= '<span class="first_page">'.__('First', 'transcend').'</span>';
		else
			$out .= '<a class="pagination-page page first_page" href="'.esc_url(get_pagenum_link(1)).'">'.__('First', 'transcend').'</a>';
		...
		if($total_pages == $current_page)
			$out .= '<span class="last_page">'.__('Last', 'transcend').'</span>';
		else
			$out .= '<a class="pagination-page page last_page" href="'.esc_url(get_pagenum_link($total_pages)).'">'.__('Last', 'transcend').'</a>';
...
if(!function_exists('cpotheme_comments_protected')){
	function cpotheme_comments_protected(){
		if(post_password_required()){
			...
			_e('This page is protected. Please type the password to be able to read its contents.', 'transcend');
			echo '</p>';
			return true;
		}
		return false;
...
if(!function_exists('cpotheme_comments_title')){
	function cpotheme_comments_title(){
		...
		if(get_comments_number() == 1)
			_e('One comment', 'transcend');
		else
			printf(__('%s comments', 'transcend'), number_format_i18n(get_comments_number()));
		echo '</h3>';
	}
}
			...
			<div class="comment-body">
				<div class="comment-title">
					<div class="comment-options">
						<?php edit_comment_link(__('Edit', 'transcend')); ?>
					...
					<div class="comment-date">
						<a rel="nofollow" href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
							<?php printf(__('%1$s at %2$s', 'transcend'), get_comment_date(),  get_comment_time()); ?>
						</a>
					</div>
				</div>
				...
				<div class="comment-content">
					<?php if($comment->comment_approved == '0'): ?>
						<span class="comment-approval"><?php _e('Your comment is awaiting approval.', 'transcend'); ?></span>
					<?php endif; ?>
...
if(!function_exists('cpotheme_comments_pagination')){
	function cpotheme_comments_pagination(){
		if(get_comment_pages_count() > 1 && get_option('page_comments')){
			...
			previous_comments_link(__('Older', 'transcend'));
			echo '</div>';
			echo '<div class="comments-next">';
			next_comments_link(__('Newer', 'transcend'));
			echo '</div>';
			echo '</div>';
		}
	}
